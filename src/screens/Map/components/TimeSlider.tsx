...
export const TimeSlider: React.FC<TimeSliderProps> = ({
  ...
  const yearToPercent = (year: number) => ((year - dataMin) / (dataMax - dataMin)) * 100;

  const handleStartChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const val = Number(e.target.value);
    const clamped = Math.min(val, localRange[1] - 25);
  ...
  const handleEndChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const val = Number(e.target.value);
    const clamped = Math.max(val, localRange[0] + 25);
  ...
  const ticks: number[] = [];
  for (let y = Math.ceil(dataMin / tickInterval) * tickInterval; y <= dataMax; y += tickInterval) {
    ticks.push(y);
  }
  ...
  const muted = darkMode ? "text-white/25" : "text-black/25";
  const trackBg = darkMode ? "bg-white/10" : "bg-black/10";
  const trackActive = darkMode ? "bg-white/50" : "bg-black/50";
  ...
  const tickColor = darkMode ? "text-white/25" : "text-black/25";

  return (
    <div className={`absolute bottom-4 left-1/2 -translate-x-1/2 z-[1000] ${bg} border ${border} shadow-sm px-5 py-3`} style={{ width: 520, maxWidth: "calc(100vw - 40px)" }}>
      <div className="flex items-center justify-center mb-2">
        <div className="flex items-center gap-1.5">
          <span className={`text-sm font-light ${text} tracking-wide tabular-nums`}>
            {localRange[0]}
          </span>
          <span className={`text-[10px] ${darkMode ? "text-white/20" : "text-black/20"} mx-1.5`}>—</span>
          <span className={`text-sm font-light ${text} tracking-wide tabular-nums`}>
            {localRange[1]}
          </span>
        </div>
      ...
      <div className="relative h-6 mx-1">
        <div className={`absolute top-2.5 left-0 right-0 h-[1px] ${trackBg}`} />
        <div
      ...
      <div className="flex justify-between mx-1 mt-0.5">
        {ticks.map((y) => (
          <span key={y} className={`text-[8px] ${tickColor} font-medium tracking-wider`}>
            {y}
          </span>
        ))}
      </div>
